// The endpoint chart's pointer readout: a block right under the plot, filled by
// whatever the pointer is on. Each lane is read on its own: hovering or tapping one
// names the day under the pointer and that lane's value on it, in that lane's
// units, over the raw data the value was computed from. There is no crosshair
// across both -- the two lanes answer different questions and are compared through
// the changes table, not by eye at a pixel. Change marks and epoch rules are
...
/** Wire the readout block to an already-rendered chart. Called after every draw, so
 *  the resize redraw does not leave the chart inert. `tipEl`'s initial content is
 *  its idle state. A readout whose raw data has not arrived yet (endpoint.ts fetches
 *  it as soon as the page is idle) shows the value at once and fills in when it
 *  lands. */
export function bindHover(
  ...
  const idle = (tipEl.dataset.idle ??= tipEl.innerHTML);

  // what the block is showing, so a detail that loads late lands on the readout
  // that asked for it and not on whatever the pointer moved to meanwhile
  let current: string | null = null;

  const reset = (): void => {
    tipEl.innerHTML = idle;
    tipEl.classList.add("idle");
    mark.innerHTML = "";
    current = null;
  };

  const render = (key: string, head: string, detail: string): void => {
    current = key;
    tipEl.innerHTML = `<div class="rh">${head}</div>${detail}`;
  ...
  const showEpoch = (i: number, ev: PointerEvent): void => {
    ...
    render(key, head, epochDetail(b3it, null, i));
    void raw.b3it().then((votes) => {
      if (votes && current === key) render(key, head, epochDetail(b3it, votes, i));
    });
  };
  ...
  // A mouse reads as it moves; a touch reads on the tap (its moves are scrolls).
  ...
  chartEl.addEventListener("pointerdown", (ev) => {
    const show = readoutAt(ev as PointerEvent);
    if (show) show();
    else reset();
  });
}
