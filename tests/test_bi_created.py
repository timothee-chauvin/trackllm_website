...
def test_save_endpoints_persists_last_seen(tmp_path, monkeypatch):
    monkeypatch.setattr(
        ue.config, "endpoints_yaml_path_bi", tmp_path / "endpoints_bi.yaml"
    )
    e = Endpoint(
        ...
        cost=(1, 1),
        cost_per_request=0.00001,
        last_seen=datetime(2026, 7, 17, tzinfo=timezone.utc),
    )
    save_endpoints_bi([e])
    text = (tmp_path / "endpoints_bi.yaml").read_text()
    assert "last_seen" in text and "2026-07-17" in text
